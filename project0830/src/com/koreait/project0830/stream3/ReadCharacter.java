...
하나의 문자로 읽을 수 있는 능력이 없다.. 따라서 영문의 경우엔 1byte차지 하므로,
읽어들인 데이터를 문자로 변경하는데 문제가 없지만, 한글의 경우엔 유니코드 기반이라 
2byte로 하나의 문자를 표현하기 때문에 read()메서드로 읽어들인 1byte는 한글을 표현할 수 없다..
해결책? 읽어들인 데이터를 대상으로 2byte씩 묶어서 문자로 이해하는 업그레이드 된 문자기반 스트림을
이용해야 한다..
